...
stopping or signalling writers. Repeat quiescence and identity checks
immediately before each deletion. For other dependent side effects, repeat
the same checks immediately before the side effect. If a writer exists, state
changes, process visibility is unavailable, or ownership is uncertain, retain
the source and report the exact gap.
...
interrupted operations, LFS/submodules, alternate/shared-object metadata, and
declared dependency artifacts. Keep credentials and sensitive filenames only in
protected local storage; handoffs contain redacted identity and pass/fail.

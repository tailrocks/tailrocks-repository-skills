...
All-work is a broad, read-only discovery request for one bound repository. Enumerate and report the exact local volumes and roots visible to the runtime: configured workspace and project roots, Git and agent worktree roots, and any roots explicitly supplied by the user. Search each declared root for `.git` directories, `.git` pointer files, bare repositories, nested repositories, and paths recorded by Git. Follow explicit Git worktree metadata and repository pointers; do not follow arbitrary symlinks beyond declared roots.
...
Query hosting APIs only for the bound repository and complete every relevant page. Record roots searched, exclusions with reasons, permission failures, unavailable volumes, API failures, incomplete pages, unresolved repository identities, and active writers. Do not claim full all-work coverage or a clean host when any in-scope root or relationship is unknown. User-data directories remain in scope even when their names resemble caches; exclude only exact paths with a stated reason.
